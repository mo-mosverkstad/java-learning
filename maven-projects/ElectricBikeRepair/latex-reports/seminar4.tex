...
The Factory pattern is applied to centralize object creation logic and decouple the system from concrete class implementations. During initialization, certain components such as data handlers must be instantiated based on configuration or runtime parameters, rather than being hardcoded.
...
The system distinguishes between two categories of exceptions: checked exceptions for business-rule violations and unchecked exceptions for internal program failures. Business-rule exceptions are thrown in the model or utility layers and are propagated unchanged through the controller to the view, where they are presented as user-friendly error messages. Unchecked exceptions are produced in the integration layer during technical failures and are caught and logged in the controller layer, while translating it into a higher-level checked exception before being propagated to the view. As a result, only business-relevant exceptions reach the view layer.

\subsubsection{Checked exception handling - InvalidTelephoneNumberException}

The \texttt{InvalidTelephoneNumberException} exception arises when a telephone number is malformed and therefore cannot be read and parsed. Since the exception represents a violation of a business rule, it is implemented as a checked exception which directly extends \texttt{Exception}. The exception contains detailed error information, in this case the malformed telephone number, to inform the user.

The checked exception \texttt{InvalidTelephoneNumberException} is defined to represent the error condition where a telephone number has an invalid format with the specific invalid input, following the naming conventions in Chapter~8. The code can be found in \href{https://github.com/mo-mosverkstad/java-learning/blob/main/maven-projects/ElectricBikeRepair/src/main/java/se/ebikerepair/util/InvalidTelephoneNumberException.java}{\nolinkurl{.../util/InvalidTelephoneNumberException.java}} \cite{sourcecode}.

The exception \texttt{InvalidTelephoneNumberException} is thrown in \texttt{TelephoneNumber} class constructor, located in utility layer. The \texttt{TelephoneNumber} takes responsibility of normalizing arbitrary formatted telephone number into E-164 format. When a telephone number is beyond recognition of any predefined format, the telephone number cannot be processed and therefore an exception \texttt{InvalidTelephoneNumberException} is thrown, whereas the logic is visualized in the listing. The implementation can be found in \href{https://github.com/mo-mosverkstad/java-learning/blob/main/maven-projects/ElectricBikeRepair/src/main/java/se/ebikerepair/util/TelephoneNumber.java}{\nolinkurl{.../util/TelephoneNumber.java}} \cite{sourcecode}.

The methods that propagate the exception must also declare the exception after the \texttt{throws} keyword, which specifies the thrown exceptions. Since the methods \texttt{findRepairOrder} as well as the internal private function \texttt{findRepairOrderIds} depends on the \texttt{TelephoneNumber} class, which throws \texttt{InvalidTelephoneNumberException}.
...
The method \texttt{createRepairOrder} invokes \texttt{searchCustomer}, which may propagate the checked exception \texttt{NotFoundCustomerException} when the specified customer does not exist.

When the checked exception \texttt{InvalidTelephoneNumberException} reaches view, it is caught alongside with other checked exceptions. The internal methods that proceeds each hardcoded action are defined in \texttt{proceedReceptionPreparationActions}, \texttt{proceedTechnicianDiagnosticActions}, and \texttt{proceedReceptionConfirmationActions} passes all their checked exceptions to the \texttt{proceedActions} method. The exception information, including error message, is extracted and displayed to the view. Since the view is simple, it only extracts the error information details from exceptions and produces a error message which is printed to the standard output. After displaying the error message, no further workflow steps are executed.
...
public void proceedActions(String telephoneNumber, String bikeSerialNumber) {
    try {
        ...
        System.out.println(ERROR_PREFIX + String.format("No customer found for telephone number: %s", e.getTelephoneNumber()));
    } catch (InvalidTelephoneNumberException e) {
        System.out.println(ERROR_PREFIX + String.format("'%s' is not a valid phone number", e.getTelephoneNumber()));
    } catch (NoExistedRepairOrderException e) {
        System.out.println(ERROR_PREFIX + String.format("No repair order found for: %s", e.getIdentifier()));
    } catch (NotFoundBikeException e) {
        System.out.println(ERROR_PREFIX + String.format("No bike found with serial number: %s", e.getSerialNumber()));
...
Unchecked exceptions are propagated through the layers without declaring it after \texttt{throws} keyword. The unchecked exception \texttt{ResourceNotFoundException} is thrown when a data resource cannot be found. The exception contains the error detail with the resource name that cannot be found. The implementation of the unchecked exception definition can be found in \href{https://github.com/mo-mosverkstad/java-learning/blob/183fd32417c1e32630f41e821e872cda2c43eda9/maven-projects/ElectricBikeRepair/src/main/java/se/ebikerepair/data/ResourceNotFoundException.java}{\nolinkurl{.../data/ResourceNotFoundException.java}} \cite{sourcecode}.
...
Reporting error messages is handled by a special and dedicated utility class \texttt{LogHandler}. The class is responsible for writing formatted logs to a specified program file \texttt{ebikerepair.log}.
...
During logging, the \texttt{logException} method is called from the controller layer whenever an internal unchecked exception is caught. The method takes an exception class of any type and extract out the error details to produce an error message. Unchecked technical exceptions are logged together with detailed diagnostic information, including exception type, message, cause, and stack trace, while business-rule violations are excluded from logging, since they represent expected execution scenarios. The detailed implementation of \texttt{logException} can be found in \href{https://github.com/mo-mosverkstad/java-learning/blob/183fd32417c1e32630f41e821e872cda2c43eda9/maven-projects/ElectricBikeRepair/src/main/java/se/ebikerepair/util/LogHandler.java}{\nolinkurl{.../util/LogHandler.java}} \cite{sourcecode}.
...
Error: '11707654322' is not a valid phone number
\end{lstlisting}
...
[May 9, 2026, 4:01:11 PM] [ERROR] se.ebikerepair.data.NoDatabaseException: No database customers found or connected
  Stack trace:
    ...
    at se.ebikerepair.view.View.proceedReceptionPreparationActions(View.java:72)
    at se.ebikerepair.view.View.proceedActions(View.java:50)
    at se.ebikerepair.startup.Main.main(Main.java:26)
    at org.codehaus.mojo.exec.ExecJavaMojo$1.run(ExecJavaMojo.java:279)
    at java.base/java.lang.Thread.run(Thread.java:1583)

[May 9, 2026, 4:01:11 PM] [ERROR] se.ebikerepair.controller.FailedOperationException: Unable to search customer due to an internal system error
...
public class DatabaseHandler{
    ...
    /**
     * Private constructor to protect it from being instantiated accidentally
     */
...
A default pricing strategy is provided for non-membership customers. The \texttt{StandardPricingStrategy} calculates the total repair cost based solely on diagnostic tasks and repair tasks, without applying any discounts. This strategy represents the baseline pricing behavior used when no membership rules apply. \href{https://github.com/mo-mosverkstad/java-learning/blob/183fd32417c1e32630f41e821e872cda2c43eda9/maven-projects/ElectricBikeRepair/src/main/java/se/ebikerepair/model/StandardPricingStrategy.java}{\nolinkurl{.../model/StandardPricingStrategy.java}} \cite{sourcecode}.
...
\href{https://github.com/mo-mosverkstad/java-learning/blob/183fd32417c1e32630f41e821e872cda2c43eda9/maven-projects/ElectricBikeRepair/src/main/java/se/ebikerepair/model/MembershipPricingStrategy.java}{\nolinkurl{.../model/MembershipPricingStrategy.java}} \cite{sourcecode}.
...
The test verifies that observer notifications are produced during the execution of the workflow by asserting the presence of characteristic notification keywords in the printed output. \href{https://github.com/mo-mosverkstad/java-learning/blob/183fd32417c1e32630f41e821e872cda2c43eda9/maven-projects/ElectricBikeRepair/src/test/java/se/ebikerepair/view/ViewTest.java}{\nolinkurl{.../view/ViewTest.java}} \cite{sourcecode}

\paragraph{Singleton design tests}

The Singleton pattern is tested using the \texttt{LogHandler} class, which is designed to only allow one single shared instance throughout the application. The tests verify that multiple calls to the \texttt{getLogger} method return the same instance and that the singleton instance is correctly initialized. \href{https://github.com/mo-mosverkstad/java-learning/blob/183fd32417c1e32630f41e821e872cda2c43eda9/maven-projects/ElectricBikeRepair/src/test/java/se/ebikerepair/util/LogHandlerTest.java}{\nolinkurl{.../util/LogHandlerTest.java}} \cite{sourcecode}
...
(*@\textcolor{blue}{ --- REPAIR ORDER MODIFICATION NOTIFICATION (BEGIN) ---}@*)
...
(*@\textcolor{blue}{ --- REPAIR ORDER MODIFICATION NOTIFICATION -(END)- ---}@*)
...
=================================
  ...
  -- Costs --
  (*@\textcolor{green}{base = 1175.00 SEK, discount = 176.25 SEK, finalCost = 998.75 SEK}@*)
