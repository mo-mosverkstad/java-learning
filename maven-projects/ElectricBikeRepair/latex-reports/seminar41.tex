...
Business-rule violations represent situations where a business rule is violated, for example searching for a non-existent customer or providing malformed input. These errors are expected to occur during normal execution
and are therefore thrown as checked exceptions. Checked exceptions are declared in method signatures and documented in Javadoc, making the possible error conditions explicit to callers and enforcing handling at compile time.
...
\paragraph{Polymorphism and other patterns}
...
In addition to the Observer pattern, further design patterns are introduced to separate responsibilities and avoid hard-coded logic. Instead of embedding behavior directly into classes, behavior is encapsulated using these patterns into separate components that can be configured or replaced. This approach makes the system easier to extend without modifying existing code.

\paragraph{Inheritance and composition}
Inheritance is used cautiously and only when a clear \textit{is-a} relationship exists. Following the  recommendations in Chapter~9, composition is preferred over inheritance in most cases to avoid tight coupling and to preserve encapsulation. Inheritance is primarily used for factoring out shared behavior, such as common controller functionality, while variation is handled through composition and polymorphism.

Throughout the implementation, design decisions are continuously evaluated based on their impact on cohesion, coupling, and encapsulation. Patterns are applied only when they provide a clear and measurable improvement to the design.
...
By extending the current implementation of the Repair Electric Bike system with exception handling, errors are handled using dedicated exception classes and are propagated through the system layers until they are handled in the view.
...
In addition, logging functionality has been introduced to track internal program failures only, making debugging easier, while user-friendly error messages are presented in the view. Business logic violations are only displayed in the view and are not written to the log, since the internal program still produces correct behavior.
...
In terms of error handling using exceptions, custom exception classes are extended from \texttt{Exception} base class to cover different groups of error conditions. Business-rule violations are represented using checked exceptions, while internal failures are represented using unchecked exceptions.
...
\begin{lstlisting}[style=javastyle, caption={searchCustomer and createRepairOrder methods in createRepairOrder class propagating exceptions}]
/**
...
public String createRepairOrder(String telephoneNumber, ProblemDTO problemDTO) throws NotFoundCustomerException, InvalidTelephoneNumberException, FailedOperationException{
    try {
        ...
        // Creating repair order using foundCustomer
    } catch (ResourceNotFoundException | InvalidResourceURIException | CannotReadFileException | CannotWriteFileException | NoDatabaseException exception){
...
When the checked exception \texttt{InvalidTelephoneNumberException} reaches view, it is caught alongside with other checked exceptions. The internal methods that proceeds each hardcoded actions are defined in \texttt{proceedReceptionPreparationActions}, \texttt{proceedTechnicianDiagnosticActions}, and \texttt{proceedReceptionConfirmationActions} passes all their checked exception to the larger \texttt{proceedActions} method where the unchecked exceptions are all handled collectively. The exception information, including error message, is extracted and displayed to the view. Since the view is simple, it only extracts the error message from the exception and printing it out into standard output. After displaying the error message, no further workflow steps are executed. 
...
public void proceedActions(String telephoneNumber, String bikeSerialNumber) {
    ...
    } catch (NotFoundCustomerException | InvalidTelephoneNumberException e) {
...
The exception is thrown during initialization of a resource handler of \texttt{JsonFileHandler} and \texttt{DatabaseHandler} in the method \texttt{initDataHandler}. The exception is propagated further through methods such as \texttt{loadDiagnosticTasks} of \texttt{DiagnosticReport} and \texttt{initRegistry} of \texttt{CustomerRegistry}. In these methods, the exception is not declared, but implicitly thrown.

Eventually, all unchecked resource exceptions are handled in controller layer and are converted into a checked exception \texttt{FailedOperationException}, which can be handled and displayed in view layer. The methods \texttt{searchCustomer} and \texttt{createRepairOrder}, for example, catches all possible unchecked exceptions and wraps them into a \texttt{FailedOperationException}, which is thrown to the view caller. The method \texttt{searchCustomer} shows the wrapping process of multiple unchecked exceptions such as \texttt{ResourceNotFoundException}, in the catch block of the method. The error message is also logged as well using the \texttt{LogHandler}
...
\begin{lstlisting}[style=javastyle, caption={Log handler class applied with singleton pattern}]
/**
...
During logging, the method \texttt{logException} can called in controller when an exception is caught in the catch block. The method takes an exception class of any type and extract out the error message, which is formatted with additional information such as created time. After that, the formatted message is written to the file.
...
\begin{lstlisting}[style=javastyle, caption={DataHandler interface}]
...
\begin{lstlisting}[style=javastyle, caption={DatabaseHandler that implements DataHandler interface}]
/**
...
\begin{lstlisting}[style=javastyle, caption={JsonFileHandler that implements DataHandler interface}]
/**
...
Based on the polymorphism design, the factory design pattern is also applied. A class name is stored as a string and passed into the \texttt{create} method, which uses Java reflect to dynamically dispatch and instantiate the desired class object. In this case, the \texttt{DataHandlerFactory} initializes a \texttt{DataHandler} object based on the class name stored in the string. This allows dynamic dispatching where the object initialization is determined during runtime, which adds flexibility during program initialization.
