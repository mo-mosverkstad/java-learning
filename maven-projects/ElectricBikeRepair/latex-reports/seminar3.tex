...
\begin{lstlisting}[style=javastyle, caption={Integration layer test setup and retrieval of an existing customer in \texttt{CustomerRegistry}}]
...
\begin{lstlisting}[style=javastyle, caption={Negative test for retrieving a non-existing customer in \texttt{CustomerRegistry}}]
@Test
...
\begin{lstlisting}[style=javastyle, caption={Test for saving and retrieving a customer in \texttt{CustomerRegistry}}]
@Test
...
\begin{lstlisting}[style=javastyle, caption={Basic initialization and state validation test for \texttt{RepairOrder}}]
...
\begin{lstlisting}[style=javastyle, caption={Test for adding a single repair task to a \texttt{RepairOrder}}]
@Test
...
\begin{lstlisting}[style=javastyle, caption={Controller layer test for searching an existing customer using \texttt{ReceptionistController}}]
...
\begin{lstlisting}[style=javastyle, caption={Negative test for searching a non-existing customer in \texttt{ReceptionistController}}]
@Test
...
\begin{lstlisting}[style=javastyle, caption={Negative test for searching with an invalid telephone number in \texttt{ReceptionistController}}]
@Test
...
\begin{lstlisting}[style=javastyle, caption={View workflow test verifying accepted repair order output}]
@Test
...
Also, negative test cases are also conducted. Providing an invalid telephone number produces a buisness logic error, which should therefore contain the keyword \texttt{Error} in the output stream. This can be done by checking the output if it contains such keyword.

\begin{lstlisting}[style=javastyle, caption={View layer negative tests verifying error output for invalid input scenarios}]
@Test
...
\begin{lstlisting}[style=javastyle, caption={Positive test cases for E164 telephone number normalization in \texttt{TelephoneNumber}}]
@Test
void testE164() {
    assertDoesNotThrow(() -> {
...
\begin{lstlisting}[style=javastyle, caption={Negative test cases for malformed telephone numbers in \texttt{TelephoneNumber}}]
@Test
