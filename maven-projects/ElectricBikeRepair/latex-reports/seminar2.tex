...
This report focuses on the \textbf{Design phase} of the \texttt{Repair Electric Bike} system and includes the design method, interaction diagrams, the resulting class diagram, and an evaluation of the design against the assessment criteria. The system design is based on the given requirements specification with both normal and alternative workflow scenarios, as well as following the \texttt{Domain Model} and \texttt{System Sequence Diagram} established earlier, the details of which can be found in Appendices~\ref{appendix:background}, \ref{appendix:domain-model} and \ref{appendix:system-sequence-diagram}.

\subsection{Requirements of Seminar 2}
The requirements of Seminar~2 are to design a complete, well-structured object-oriented architecture for the \texttt{Repair Electric Bike} system. The seminar focuses on applying established principles from the course literature, particularily in Section~4.1 and Chapter~5 of \texttt{A First Course in Object-Oriented Development}.

The design must satisfy the following requirements:

\begin{itemize}
  \item \textbf{Layered architecture following the MVC and Layer patterns}.  
        The program must be organised into appropriate layers with correct direction of dependencies and no mixing of responsibilities.

  \item \textbf{High cohesion, low coupling, and strong encapsulation}.  
        The design shall demonstrate a well-defined public interface, clear responsibilities, and minimal dependencies between components.

  \item \textbf{Complete coverage of the entire Repair Electric Bike scenario}.  
        All basic and alternative flows must be designed, including the startup sequence (the \texttt{main} method).

  \item \textbf{Interaction diagrams for all behaviour}.  
        The Result chapter must include sequence or communication diagrams covering \emph{every} system operation, including alternative flows and the startup sequence.

  \item \textbf{A class diagram consistent with the interaction diagrams}.  
        The diagram must include all classes used in the design, but may omit excessive detail for clarity.
\end{itemize}
...
This report therefore describes the design method, the resulting class and interaction diagrams, as well as a discussion evaluating the design according to the required principles.
...
In the last step, the new design in code is implemented, with the help from class diagram and sequence diagrams for each operation. The class diagram helps to create the skeleton code with each class name, whereas the system operation helps to fill out the relevant implementation steps of each system operation. The design is implemented in code to verify that the interactions and dependencies defined in the diagrams are feasibly designed.

By implementing the design in code, the code editor VSCode is used since it offers syntax highlightening and variable autocompletion. For compiling and testing the code, Maven is used for full codebase compilation, handle project dependencies and automate build tasks as well as handling testing using JUnit test cases.
...
Every system operation defined in the SSD is converted into a communication diagram, presented in the same order as in the SSD. This chapter therefore describes the design of every system operation, including the interaction diagrams and class diagram.

\subsection{The \texttt{searchCustomer} system operation}

The first operation, \texttt{searchCustomer}, retrieves a customer using a telephone number as input. In the searchCustomer case, the \texttt{View} class calls the method \texttt{searchCustomer} with a phone number in free format on \texttt{ReceptionistController} can be first written. Since a customer object needs to be returned, the receptionist controller class should find the corresponding customer. This can be done by calling \texttt{CustomerRegistry} that stores and handles customer informations, the function \texttt{find} is called by passing a phone number in canonical E164 format. In this case, the given phone must be converted from any format into canonical E164 format through \texttt{TelephoneNumber} class, before handing it into CustomerRegistry. The full process of is visualized in Figure~\ref{fig:search-customer}.
...
% todo: this figure should be updated.
...
The full class diagram can be visualized in Figure~\ref{fig:class-diagram}, which represents a layered system architecture with five main packages: startup, view, controller, model, and integration. The data package is omitted since the program currently does not store any data into secondary storage, such as disk and database. Each package takes a specific and unambiguous responsibility inside the system, which ensures separation of concerns and maintainability. Additionally, methods with public, protected and default visibility are displayed in the class diagram, while the private ones are hidden, in order to keep the diagram compact with relevant details. Class member fields are omitted from the diagram as well, since all class members are kept private to protect themselves from being corrupted by external factors.

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\textwidth]{figures/class-diagram.png}
    \caption{A full class diagram containing the necessary package for each layer. Classes in each layer are visualized as well as their relationships. Class member fields and private methods are not shown in order to make the diagram compact with relevant details.}
...
\subsection{Evaluation of high cohesion, low coupling, and good encapsulation}

The design produces high cohesion, low coupling, and good encapsulation.

\paragraph{High cohesion}
High cohesion means that each class has a single, clearly defined responsibility and contains only behavior that belongs to its abstraction. The design achieves this through several structural choices:

\begin{itemize}
  \item \textbf{Entities model one domain abstraction.}  
        The \texttt{RepairOrder} entity is cohesive because it encapsulates only the logic associated with the lifecycle of a repair order—its creation, acceptance, rejection, and state transitions. The detailed implementations of updating problems, adding record diagnostic results, and adding repair tasks are intentionally delegated to dedicated classes that handle each detailed responsibility such as \texttt{Problem}, \texttt{DiagnosticReport}, and \texttt{RepairTask}. Additionally, \texttt{RepairOrder} does not contain any external responsibilities outside the scope of repair order, such as actions related to printing, telephone formatting, or JSON processing. Instead, these concerns properly belong to other parts of the system.

  \item \textbf{Supporting classes serve only their intended purpose.}  
        The \texttt{Printer} class focuses strictly on formatting and printing \texttt{RepairOrder} summaries. It does not query registries, compute costs, or change order state, which ensures a clean separation between presentation and domain logic.

  \item \textbf{DTOs does not contain any behavior except representation.}  
        Records such as \texttt{CustomerDTO}, \texttt{BikeDTO}, and \texttt{RepairOrderDTO} are immutable and does not contain any business logic. Their only purpose is to transfer data from the model to the view, while preserving cohesion in both layers.

  \item \textbf{Registries manage only their own data.}  
        \texttt{CustomerRegistry} and \texttt{RepairOrderRegistry} are responsible only for storing, retrieving, and updating domain objects. However, they do not interpret and take responsibility in high-level business rules, phone number validations, or perform calculations. As a result, registry code remains cohesive and easy to maintain.

  \item \textbf{Utility classes do only one focused job.}  
        The \texttt{TelephoneNumber} class is responsible solely for validating and normalizing Swedish telephone numbers. As a result, it does not contain any logic related to customer search, registries, or formatting of any other domain concepts. As phone-number formats are changed, only this class is updated.

  \item \textbf{Value objects encapsulate one concept.}  
        The \texttt{Cost} class encapsulates the monetary amount and currency, without mixing in unrelated behavior such as formatting, printing, or persistence. This reduces scattered primitive values that are harder to maintain. Additionally, all operations related to combining costs are placed inside \texttt{Cost}, ensuring that no other class repeats the same logic.
\end{itemize}
...
Low coupling means keeping as few dependencies to other classes and that a program works better when its components rely on each other as little as possible. The design applies this principle in several ways:

\begin{itemize}
  \item \textbf{Controllers never modify entity internals.}  
        Controllers depend only on high-level operations, never on the internal structure of model classes. For example, the \texttt{ReceptionistController} never traverses fields of \texttt{RepairOrder}. Instead, it calls domain methods such as \texttt{accept()} and \texttt{reject()} which hiding all internal low-level logic mechanisms. Similarly, the technician does not inspect internal diagnostic task lists but calls \texttt{updateDiagnosticResult()}, letting the model to decide how the update is performed.
  \item \textbf{Entities does not know about controllers or view.}  
        The model layer remains independent of higher layers. For instance, \texttt{RepairOrder} has no references to \texttt{ReceptionistController} or \texttt{TechnicianController}, and \texttt{DiagnosticReport} does not rely on how the view displays its data. This allows the model with core business logic to change freely. For instance, switching from an \texttt{ArrayList} to a priority queue in \texttt{RepairTaskCollection} would not affect controllers or the view. The reversed also applies: replacing a simple hardcoded view with a GUI interface view does not either affect the underlying model and its logic.
  \item \textbf{No logic in View.}  
        The \texttt{View} only forwards user input and prints results. As a result, it never computes costs, formats telephone numbers, or manipulates repair orders. For example, when the receptionist enters a phone number, the view simply passes it to the controller. All normalization into E.164 format happens in the \texttt{TelephoneNumber} class, ensuring the view layer stays thin and decoupled from business rules.
  \item \textbf{RegistryCreator only creates registries.}  
        The \texttt{RegistryCreator} is responsible only for constructing registries, but not for maintaining them or performing domain logic. It simply instantiates \texttt{CustomerRegistry} and \texttt{RepairOrderRegistry} and provides access through getters. This separation allows the model and integration layers to evolve - e.g., adding in-memory cache storages, database connections, or lazy loading proxy objects in the registries - without modifying the controller or startup code.
  \item \textbf{DTOs isolate the model from external dependencies.}  
        Because DTOs such as \texttt{CustomerDTO}, \texttt{RepairOrderDTO}, and \texttt{DiagnosticTaskDTO} are immutable, controllers and views cannot accidentally modify model state. For example, the view may display a \texttt{RepairOrderDTO}, but modifying it has no effect on the underlying \texttt{RepairOrder} entity. This reduces coupling by preventing accidental back-references or shared mutable data.
  \item \textbf{Utility classes encapsulate cross-cutting logic.}  
        The \texttt{TelephoneNumber} class encapsulates all parsing and validation rules for Swedish phone numbers. Neither controllers nor registries need to  know how many digits a subscriber number must have or how to handle "00" prefixes. If the formatting rules change, only this utility class needs to be updated, leaving all dependent code untouched.
\end{itemize}
...
\paragraph{High encapsulation} Encapsulation refers to hiding the internal details of a class by exposing only its public interface, while keeping the private implementation code inaccessible to the rest of the program. And it is also strengthened through the consistent use of immutable DTOs, which prevent the View from mutating internal system state.

\begin{itemize}
  \item \textbf{Private class fields} All of the member variables in the class are kept private. This protects the object invariants from external corruption.
  \item \textbf{Immutable DTOs} \texttt{CustomerDTO}, \texttt{BikeDTO}, and \texttt{RepairOrderDTO} expose only accessors and not mutators ensuring that external layers cannot corrupt the model.
  \item \textbf{Modifications only via entity methods} Model objects also never leak internal references. As an example, \texttt{RepairOrder} does not expose its internal detailed functions directly. Instead, all state transitions occur through dedicated methods such as \texttt{addRepairTask()}, \texttt{accept()}, or \texttt{reject()}, which prevents accidental external modification of the class and ensures that all invariants are checked within the model layer.
\end{itemize}

 Putting it together, by using these structural choices, these implentations result in a design where responsibilities are clear, coupling remains minimal, and the system maintains a strong and robust encapsulation boundary.
...
This section evaluates how the design handles types, object usage, static elements, data integrity, and naming conventions in accordance with the Seminar~2 assessment criteria.

\paragraph{Correct use of parameters, return values, and type clarity}
...
\paragraph{Use of objects instead of primitive data}
...
\paragraph{Avoidance of static methods and attributes}
...
\paragraph{Keeping objects intact and protecting internal state}
...
\paragraph{Compliance with Java naming conventions}
