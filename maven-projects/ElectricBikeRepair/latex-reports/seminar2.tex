...
This report concerns the design of a software system for managing repair processes in an electric bike workshop. The system design is based on given requirement specification with both normal and alternative workflow scenarios.

\subsection{Requirements of Seminar 2}
The objective of Seminar~2 is to design a complete, well‑structured 
object-oriented architecture for the \texttt{Repair Electric Bike} system. 
The seminar focuses on applying established principles from the course 
literature, in particular Section~4.1 and Chapter~5 of 
\texttt{A First Course in Object-Oriented Development}.
...
In order to design the software, both the domain model (DM) and system sequence diagram (SSD) is required for further design and can be produced from requirement analysis.
...
Alternative flows includes invalid written telephone number and rejected repair order. Workflow for invalid telephone number can be visualized in Figure~\ref{fig:system-sequence-diagram}.
...
Based on the domain model (DM) and system sequence diagram (SSD) from analysis, the system design can be carried out by first dividing the system into the layered architecture pattern based on MVC and therefore designing one system operation at a time while aiming high cohersion, low coupling and high degree of encapsulation. 

\subsection{Step 1. Use the patterns MVC and layer}

The first step involves usage of layered architectural MVC pattern, which includes six layers: view, controller, model, integration, data and startup, which can be visualized in Figure~\ref{fig:layered-architecture}. View layer concerns interactions with user while core logic are handled in model layer. Controller layer acts as a bridge between the view and the model and carries the necessary system operations. By calling each system operation, the equivalent operations in the model and integration are handled. The integration layer acts as a middle ground between the core logic and data layer, which in turn is responsible for storing data. Finally, the startup layer handles program initialization of the rest five layers and serves as the main function.
...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{The layered MVC architecture pattern is divided in six layers with each package corresponding to each layer}
    \label{fig:layered-architecture}
  \end{center}
\end{figure}

The bike repair system implements the MVC pattern by dividing the entities in domain model as classes into different layers. Both View and Controller layer introduces its own classes.
In terms of Controller layer, two classes are created: \texttt{ReceptionistController} and \texttt{TechnicianController}. The reason is that both roles has different authorizations, such as a technician cannot reject a \texttt{RepairOrder}. Since both classes has some common methods and properties, they share as a result a common superclass \texttt{Controller}. The \texttt{Controller} superclass only provides a common method \texttt{findRepairOrder} which are both used by \texttt{ReceptionistController} and \texttt{TechnicianController} classes.

Both \texttt{ReceptionistController} and \texttt{TechnicianController} are collectively instantiated through \texttt{ControllerCreator}, which centralizes controller construction and facilitates scalability in case additional controller roles are introduced in the future.

Similar pattern is also shown for registry classes. Both \texttt{CustomerRegistry} and \texttt{RepairOrderRegistry} are collectively created using \texttt{RegistryCreator}, which handles their initialization.
...
High cohersion, low coupling, and a high degree of encapsulation with a small, well-defined public interface are several important OOD guidelines.

When a new functionality is added, method should only be created that these principles are satisfied as fully as possible. For instance, a class or a method should be responsible for its dedicated and solemn functionality with as less connection to its neighbour classes as possible.
...
Objects are favoured over primitive data types as well as avoiding static members, since primitive data and static members are not object oriented which causes harder to acheive theese three guidelines. For instance, \texttt{Cost} is modeled as its own class instead of using primitive numbers, since cost include amount, currency and probably future metadata.
...
Following the steps of the method, sequence diagram for each system operation and class diagrams could be drawn, which can function as blueprints for implementing the code.

\subsection{The \texttt{searchCustomer} system operation}

\textbf{In the first step}, the layered architecture pattern based on MVC-pattern is used. Each layer in the pattern is mapped into each package. Since the project involves internal computational logic, database/disk writing as well as interactive view interface to user, the program is divided into different layers where each of them takes responsibility of one concern. In this case, different concerns can be cleanly separated into its respective layers, such as data layer focusing on file handling and model for internal computational logic.


\textbf{In the second step}, each system operation from the SSD is translated into a communication diagram. These diagrams comes in the similar order as the SSD shows. The first operation is searchCustomer, which fetches a customer by a telephone number as an input. Additionally, the view package is not designed in this process. Instead, the view package only contains a single class View, which is a placeholder for a real view.
...
Additionally, in order for the program to produce results, the start sequence must also be implemented. During initialization, all relevant classes in each layer should be initialized during startup. This includes initializing \texttt{RegistryCreator}, \texttt{Printer}, \texttt{ControllerCreator} and \texttt{View} class which represents the fundamental classes of each layer. In integration layer, \texttt{RegistryCreator} starts up \texttt{CustomerRegistry} and \texttt{CustomerRegistry}, as well as \texttt{ControllerCreator} starting up \texttt{TechnicianController} and \texttt{TechnicianController}.

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\textwidth]{figures/startup.png}
    \caption{A simplified and tailored diagram for program startup after implementing \texttt{searchCustomer} system operation. The diagram does not represent the final design of startup, but only shows the relevant classes for the current system operation.}
    \label{fig:startup}
  \end{center}
\end{figure}


\textbf{The third step} involves aiming for the three guidelines in OOD: encapsulation with a small, well-defined public interface, high cohersion and low coupling.
...
In the \texttt{searchCustomer} operation several important design principles are applied. First, the telephone number provided by the view is accepted in any arbitrary format, but must be normalized by converting into canonical E164 form through the \texttt{TelephoneNumber} class. This ensures that data does not "appear out of nothing" and that both original and transformated values are explicit written.
...
Furthermore, in integration layer, \texttt{RegistryCreator} starts up \texttt{CustomerRegistry} and \texttt{CustomerRegistry}, as well as \texttt{ControllerCreator} initializes \texttt{TechnicianController} and \texttt{TechnicianController}.
...
The full class diagram can be visualized in Figure~\ref{fig:class-diagram}, which represents a layered system architecture with five main packages: startup, view, controller, model, and integration. The data package is omitted since the program currently does not store any data into secondary storage, such as disk and database. Each package takes a specific and unambigious responsibility inside the system, which ensures separation of concerns and maintainability.
...
The view package contains the \texttt{View} class, which acts as the user interface layer. Currenlty, the view class is hardcoded and communicates directly with the control layer by calling system operations as methods in classes of control layer.
...
The design follows the MVC and Layer patterns, by separation into View, Controller, and Model. The View contains only user-interaction logic, which are currently hard-coded, while all system-operation logic is placed in the controllers, and business logic remains in the Model. Additionally, view-related code does not appear in other layer, which confirms that all the concerns are separated into layers that focus on one solemn and unambigious responsibility.
...
Primitive data is used only when semantically appropriate, for example strings for names and description. In most cases, the design introduces dedicated value objects to avoid primitive obsession. For example, \texttt{ProblemDTO} groups problem description with bike information and \texttt{Cost} encapsulates amount and currency. These choices improve cohesion, reduce duplicated logic, and make method signatures more stable.
...
Overall, the report method clearly explains the design by following the OOD approach using MVC and layer patterns, designing one system operation at a time, applying cohersion-coupling-encapsulation guidelines and finally maintaining a class diagrams. These steps are explicitly described and in ocorrect order, which makes design process clean and easy to follow.

The result chapter is also clean and explains the outcome of each methodological step, including each system operation. Each system operation design is described using a corresponding interaction diagram and a short explanation of how messages flow across layers. Additionally, in the end of result section, a full class diagram and the startup sequence are layed out, which shows the overeall mechanism of the program. 

Additionally, a discussion section evaluates the solution against the assessment criteria. It addresses MVC layered architecture, cohersion, coupling, encapsulation, DTO immutablility and the separation of concerns across different layers in the architecture. It also includes strengths and limitations, such as the insight that the controller could be split into two different based on technician and receptionist roles.
